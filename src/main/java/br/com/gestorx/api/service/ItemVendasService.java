...
public class ItemVendasService {
    ...
    public ItemVendas salvar(ItemVendas itemVenda) {
        if (itemVenda.getEstoque() != null) {
            ...
            // Converter BigDecimal para Integer para comparar
            int quantidadeSolicitada = itemVenda.getQuantidade().intValue();
            
            // Comparar Integer com Integer
            if (estoque.getQuantidadeDisponivel() < quantidadeSolicitada) {
                throw new RuntimeException("Quantidade em estoque insuficiente. Disponível: " 
                    + estoque.getQuantidadeDisponivel() + ", Solicitado: " + quantidadeSolicitada);
            }
    ...
    public ItemVendas atualizar(Long id, ItemVendas itemVendaAtualizado) {
        ...
        if (itemExistente.isPresent()) {
            ...
            if (itemVendaAtualizado.getQuantidade() != null && item.getEstoque() != null) {
                Estoque estoque = estoqueRepository.findById(item.getEstoque().getId())
                        .orElseThrow(() -> new RuntimeException("Estoque não encontrado"));
                
                int quantidadeSolicitada = itemVendaAtualizado.getQuantidade().intValue();
                if (estoque.getQuantidadeDisponivel() < quantidadeSolicitada) {
